...
def _load_stackcont(imagename,new_stampsize,coords,verbose,weightstack): 
    ...
    if verbose >= 3:
        print('The coordinates found in this cube are: %s'%coords)
...
def three_panel_fig(stack_image_file, stack_cube_file, figname , unit='mJy'):
    ...
    figtitle = fig.suptitle(r'N = %i, $\bar{\sigma}_{all}$ = %.1f %sJy, $\sigma_{stack}$ = %.1f %sJy'  %(stack_cube_data.shape[0],cube_std,unit_text,np.nanstd(edge_im),unit_text), fontsize=20, y=1.05)
    #if fit == 0:
    #   figtitle = fig.suptitle(r'N = %i, $\bar{\sigma}_{all}$ = %.1f %sJy, $\sigma_{stack}$ = %.1f %sJy'  %(stack_cube_data.shape[0],cube_std,unit_text,np.nanstd(edge_im),unit_text), fontsize=20, y=1.05)
...
def plots_cont(verbose):
    ...
    stack_fig(stack_image_file = image_file, stack_cube_file = cube_file, figname = figname , unit=unit)
    one_panel_fit(stack_image_file = image_file, stack_cube_file = cube_file, figname = figname , unit=unit)
    figname = 'mean_stack_smoothing.png'
    ...
    stack_fig(stack_image_file = image_file, stack_cube_file = cube_file, figname = figname , unit=unit)
    figname = 'median_stack_smoothing.png'
    three_panel_fig(stack_image_file = image_file, stack_cube_file = cube_file, figname = figname , unit=unit)
...
##
################################################################# 
# dirname='Subsamples/codes_test/example/cont-slice'
# incubes=['Subsamples/codes_test/example/cont-slice/4l-10mJy-3s.fits']
# incat='lines.csv'
# stampsize = 20
verbose = 2
